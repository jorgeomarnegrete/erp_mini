...
import { CreditCard, Edit, Trash2, Plus, CheckCircle2, X, XCircle } from 'lucide-react';
...
export default function TiposDoc() {
  ...
  const fetchData = async () => {
    try {
      const res = await api.get('/api/tipos-doc');
      setTipos(res.data);
    } catch (error) {
  ...
  const handleSave = async (e) => {
    ...
    try {
      if (modalMode === 'create') {
        ...
        delete payload.id;
        await api.post('/api/tipos-doc', payload);
      } else {
        const payload = { ...formData };
        delete payload.id;
        await api.put(`/api/tipos-doc/${formData.id}`, payload);
      }
  ...
  const handleDelete = async (id) => {
    if (window.confirm("¿Seguro que deseas eliminar el tipo de documento? Podría afectar clientes cargados.")) {
      try {
        await api.delete(`/api/tipos-doc/${id}`);
        fetchData();
      } catch (err) {
    ...
    <div className="bg-white rounded-2xl shadow-xl border border-gray-100 overflow-hidden relative">
      <div className="px-8 py-6 border-b border-gray-100 bg-gradient-to-r from-indigo-50 to-white flex items-center justify-between">
        <div className="flex items-center">
          ...
          <h2 className="text-2xl font-bold text-gray-800 tracking-tight">Tipos de Documento (ARCA)</h2>
        </div>
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md p-6 transform transition-all">
            <div className="flex justify-between items-center mb-5">
              <h3 className="text-xl font-bold text-gray-900 flex items-center pb-2 border-b w-full mt-2">
                ...
                {modalMode === 'create' ? 'Nuevo Tipo de Documento' : 'Edición de Documento'}
              </h3>
