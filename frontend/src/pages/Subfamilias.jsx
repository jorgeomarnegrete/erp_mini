...
import { Layers, Edit, Trash2, Plus, CheckCircle2, XCircle, X } from 'lucide-react';
...
export default function Subfamilias() {
  ...
  const [formData, setFormData] = useState({ id: null, nombre: '', activo: true });
  const [formError, setFormError] = useState('');

  const fetchData = async () => {
    try {
      const res = await api.get('/api/subfamilias');
  ...
  const handleSave = async (e) => {
    ...
    try {
      ...
      delete payload.id;
      if (modalMode === 'create') await api.post('/api/subfamilias', payload);
      else await api.put(`/api/subfamilias/${formData.id}`, payload);
  ...
  const handleDelete = async (id) => {
    if (window.confirm("¿Seguro que deseas eliminar esta Subfamilia? Afectará a los productos asignados.")) {
      try {
        await api.delete(`/api/subfamilias/${id}`);
        fetchData();
      } catch (err) {
      ...
      <div className="px-8 py-6 border-b border-gray-100 bg-gradient-to-r from-indigo-50 to-white flex items-center justify-between">
        ...
        <div className="flex items-center space-x-4">
          ...
          <button onClick={openCreateModal} className="flex items-center bg-indigo-600 hover:bg-indigo-700 text-white px-4 py-2 rounded-lg font-medium transition-colors shadow-sm">
            <Plus className="w-5 h-5 mr-1" /> Nueva Subfamilia
          </button>
        </div>
      </div>
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-left border-collapse">
          ...
          <tbody className="divide-y divide-gray-100/60">
            {subfamilias.map((s) => (
              <tr key={s.id} className="hover:bg-indigo-50/30 transition-colors duration-150">
                <td className="px-8 py-4 whitespace-nowrap text-sm text-gray-500 font-medium">#{s.id}</td>
                <td className="px-8 py-4 whitespace-nowrap">
                  <span className="text-base font-extrabold text-indigo-900 tracking-tight bg-indigo-50 px-2 py-0.5 rounded">{s.nombre}</span>
                </td>
                <td className="px-8 py-4 whitespace-nowrap text-center">
                  {s.activo ? (
                    ...
                    <span className="inline-flex items-center px-3 py-1 rounded-full text-xs font-bold bg-gray-100 text-gray-500 border border-gray-200">
                      <XCircle className="w-4 h-4 mr-1.5" /> Retirada
                    </span>
                  )}
                </td>
                <td className="px-8 py-4 whitespace-nowrap text-center space-x-3">
                  <button onClick={() => openEditModal(s)} className="text-indigo-600 hover:text-indigo-900 transition-colors" title="Editar">
                    <Edit className="w-5 h-5 inline" />
                  </button>
                  <button onClick={() => handleDelete(s.id)} className="text-red-500 hover:text-red-700 transition-colors" title="Eliminar Permanente">
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md p-6 transform transition-all">
            <div className="flex justify-between items-center mb-5">
              <h3 className="text-xl font-bold text-gray-900 flex items-center pb-2 border-b w-full mt-2">
                ...
                {modalMode === 'create' ? 'Registro de Subfamilia' : 'Modificación de Subfamilia'}
              </h3>
            ...
            <form onSubmit={handleSave} className="space-y-4">
              <div>
                ...
                <input type="text" required value={formData.nombre} onChange={(e) => setFormData({...formData, nombre: e.target.value})} className="w-full px-4 py-2 rounded-xl border border-gray-300 focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none font-medium" placeholder="Ej. Rebozados, Congelados" />
              </div>
              <div className="flex items-center pt-4 border-t border-gray-100 mt-4 ml-1">
                ...
                <label htmlFor="isActivo" className="ml-2 block text-sm font-bold text-gray-800 cursor-pointer">Activa</label>
              </div>
