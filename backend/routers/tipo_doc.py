...
async def create_tipo(tipo_in: TipoDocCreate, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
...
async def update_tipo(record_id: int, tipo_in: TipoDocUpdate, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
