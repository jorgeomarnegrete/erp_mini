...
async def create_cat(cat_in: CategoriaCreate, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
...
async def update_cat(record_id: int, cat_in: CategoriaUpdate, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
...
async def delete_cat(record_id: int, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    db_record = crud_cat.get_by_id(db, record_id=record_id)
    if not db_record:
