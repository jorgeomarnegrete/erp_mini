...
async def create_lista(tipo_in: ListaPrecioCreate, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
...
async def update_lista(record_id: int, tipo_in: ListaPrecioUpdate, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
...
async def delete_lista(record_id: int, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    db_record = crud_lp.get_by_id(db, record_id=record_id)
    if not db_record:
